...
class WallpapersAdapter(var wallpapersList: List<WallpapersModel>, private val clickListener: (WallpapersModel) -> Unit) : RecyclerView.Adapter<WallpapersAdapter.WallpapersViewHolder>() {
    ...
    class WallpapersViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(wallpapers: WallpapersModel, clickListener: (WallpapersModel) -> Unit) {
            //Load the Image
            Glide.with(itemView.context).load(wallpapers.url).listener(
